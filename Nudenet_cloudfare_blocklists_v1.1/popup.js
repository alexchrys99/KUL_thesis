...
function updateCounters() {
    chrome.storage.local.get([
        ...
        const currentBlockCount = document.getElementById('currentPageBlockCount');
        const totalBlockCount = document.getElementById('totalBlockCount');
        
        if (currentPageCount) {
            currentPageCount.textContent = data.currentPageFilterCount || 0;
        }
        if (totalCount) {
            totalCount.textContent = data.totalFilterCount || 0;
        }
        if (currentBlockCount) {
            currentBlockCount.textContent = data.currentPageBlockCount || 0;
        }
        if (totalBlockCount) {
            totalBlockCount.textContent = data.totalBlockCount || 0;
        }
    });
}
...
// No need to query tabs since we're not actually changing anything
// Remove the chrome.tabs.query section that notifies tabs about changes
