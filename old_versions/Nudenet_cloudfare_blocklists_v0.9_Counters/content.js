...
async function processImage(img) {
    ...
    try {
        ...
        const result = await makeRequest('https://pack-gaps-colour-rate.trycloudflare.com/predict', {
            method: 'POST',
...
function traverseShadowDOM(root) {
    // Handle media in the current root
    root.querySelectorAll('img, video, [style*="background"]').forEach((node) => {
    ...
    observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            ...
            mutation.addedNodes.forEach((node) => {
                ...
                node.querySelectorAll('img, video').forEach((mediaNode) => {
                    if (mediaNode.nodeName === 'IMG') {
                        processImage(mediaNode);
                    } else if (mediaNode.nodeName === 'VIDEO') {
                        processVideo(mediaNode);
                    }
                });
            });
        });
