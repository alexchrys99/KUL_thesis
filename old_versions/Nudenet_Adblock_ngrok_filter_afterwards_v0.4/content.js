...
function blockAds() {
    // Example: Block ad-related elements
    document.querySelectorAll('img, iframe, script').forEach((element) => {
...
// Function to process media for NSFW content
async function processMedia() {
    // Process images, GIFs, and videos for NSFW content
...
async function processImage(img) {
    ...
    try {
        ...
        const response = await fetch('https://5952-178-51-62-246.ngrok-free.app/predict', {
            method: 'POST',
        ...
        setTimeout(() => processImage(img), 5000); // Retry after 5 seconds
    }
}
...
async function processGIF(gif) {
    ...
    try {
        ...
        const apiResponse = await fetch('https://5952-178-51-62-246.ngrok-free.app/predict', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ base64_image: imageDataUrl })
        });
...
function measurePageLoadTime() {
    ...
    if (!timing) {
        console.error("Navigation timing data not available.");
        return;
    }

    // Calculate the page load time using newer properties
    const loadTime = timing.domContentLoadedEventEnd - timing.startTime;

    // Get the current site's URL
    const siteUrl = window.location.hostname; // e.g., "www.youtube.com"

    // Log the page load time and site URL
    console.log(`Page load time for ${siteUrl}:`, loadTime, "ms");

    // Send the page load time and site URL to the background script
...
// Measure page load time when the page is fully loaded
window.onload = measurePageLoadTime;
observeMedia();
